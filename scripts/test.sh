...
# SecChain Test Script
# Comprehensive testing for SecChain CLI tool
...
# Check if SecChain binary exists
check_binary() {
    print_status "Checking if SecChain binary exists..."
    
    if [ -f "./secchain" ]; then
        test_result "Binary exists" "0"
    else
        print_status "Building SecChain binary..."
...
test_config_commands() {
    ...
    # Test config show
    ./cc config show > /dev/null 2>&1
    test_result "Config show" "$?"
    
    # Test config set
    ./cc config set test_mode test_value > /dev/null 2>&1
    test_result "Config set" "$?"
    
    # Test config reset
    ./cc config reset > /dev/null 2>&1
...
test_scan_commands() {
    ...
    ./cc scan --pkg lodash --version 4.17.15 --ecosystem node > /dev/null 2>&1
    test_result "Package scan (lodash)" "$?"
    
    # Test package scanning with different output format
    ./cc scan --pkg express --version 4.18.2 --ecosystem node --format json > /dev/null 2>&1
    test_result "Package scan (express, JSON)" "$?"
    
    # Test package scanning with minimal format
    ./cc scan --pkg requests --version 2.28.1 --ecosystem python --format minimal > /dev/null 2>&1
...
test_error_handling() {
    ...
    ./secchain invalid-command > /dev/null 2>&1
    if [ $? -ne 0 ]; then
    ...
    ./cc scan --pkg nonexistent-package-12345 --version 1.0.0 --ecosystem node > /dev/null 2>&1
    if [ $? -ne 0 ]; then
...
test_cve_detection() {
    ...
    output=$(./cc scan --pkg lodash --version 4.17.15 --ecosystem node --format table 2>/dev/null)
...
test_json_output() {
    ...
    output=$(./cc scan --pkg express --version 4.18.2 --ecosystem node --format json 2>/dev/null)
...
test_config_file() {
    ...
    if [ ! -d "$HOME/.config/secchain" ]; then
        ./cc config show > /dev/null 2>&1
        if [ -d "$HOME/.config/secchain" ]; then
    ...
    if [ -f "$HOME/.config/secchain/config.toml" ]; then
...
performance_test() {
    ...
    # Time a simple scan
    start_time=$(date +%s.%N)
    ./cc scan --pkg express --version 4.18.2 --ecosystem node > /dev/null 2>&1
    end_time=$(date +%s.%N)
